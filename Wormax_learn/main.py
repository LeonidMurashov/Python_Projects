...
from screen_consts import GRAB_REGION, WIDTH, HEIGHT, max_x, max_y, min_x, min_y
...
def main():
    ...
    while (True):
        ...
        if not paused:
            ...
            last_time = time.time()
            img = roi(cv2.cvtColor(cv2.resize(img, (WIDTH, HEIGHT)), cv2.COLOR_RGB2GRAY), [vertices])
