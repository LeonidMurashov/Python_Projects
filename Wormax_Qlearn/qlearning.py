...
test_model_path = 'models/model-{}/qlearning.ckpt-14000'.format(MODEL_NUMBER)
# Atari game to learn
...
summary_dir = "D:\Python\Wormax_Qlearn\log"
summary_interval = 100
checkpoint_path = 'D:\Python\Wormax_Qlearn\models/model-{}\qlearning.tflearn.ckpt'.format(MODEL_NUMBER)
checkpoint_interval = 2000
# Number of episodes to run gym evaluation
num_eval_episodes = 1000#100
...
def build_dqn(num_actions, action_repeat):
	...
	Building a DQN.
	"""
	inputs = tf.placeholder(tf.float32, [None, action_repeat, WIDTH, HEIGHT])
...
class AtariEnvironment(object):
	...
	def __init__(self, gym_env, action_repeat):
		...
		self.gym_actions = range(len(gym_env.actions))
	...
	def get_preprocessed_frame(self, observation):
		...
		"""
		return observation#resize(rgb2gray(observation), (WIDTH, HEIGHT))
	...
	def step(self, action_index):
		...
		s_t1 = np.empty((self.action_repeat, HEIGHT, WIDTH))
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions,
	...
	while T < TMAX:
		...
		while True:
			...
			s_t = np.reshape(s_t, newshape=[ 4, 160, 90])
			#s_t = np.array(s_t).reshape(-1, 4, 160, 90)
			...
			s_t1 = np.reshape(s_t1, newshape=[ 4, 160, 90])
			...
			if t % checkpoint_interval == 0:
				if not os.path.exists("D:/Python/Wormax_Qlearn/models/model-{}".format(MODEL_NUMBER)):
					os.mkdir("D:/Python/Wormax_Qlearn/models/model-{}".format(MODEL_NUMBER))
				saver.save(session, "D:/Python/Wormax_Qlearn/models/model-{}/qlearning.ckpt".format(MODEL_NUMBER), global_step=t)
				print("Progress saved. iter =", t)
...
def train(session, graph_ops, num_actions, saver):
	...
	envs = [Environment() for i in range(n_threads)]
	...
	writer = writer_summary(summary_dir + "/qlearning_model-{}".format(MODEL_NUMBER), session.graph)
